    ...
    _AuthorsService.getAllauthors().subscribe((authors) => {
      ...
      this.route.queryParamMap.subscribe((params) => {
        ...
        if (this.currentURL == '/authors') {
          this.newauthors = this.authors.filter(
            (elm) => elm.firstName.toLowerCase() == this.newsearch.toLowerCase()
          );
        }
    ...
    _BooksService.getAllbooks().subscribe((books) => {
      ...
      this.route.queryParamMap.subscribe((params) => {
        ...
        if (this.currentURL == '/books') {
          this.newbooks = this.books.filter(
            (elm) => elm.title.toLowerCase() == this.newsearch.toLowerCase()
          );
        }
    ...
    _CatagoryService.getAllcategories().subscribe((catagories) => {
      ...
      this.route.queryParamMap.subscribe((params) => {
        ...
        if (this.currentURL == '/categories') {
          this.newcatagories = this.catagories.filter(
            (elm) => elm.name.toLowerCase() == this.newsearch.toLowerCase()
          );
        }
